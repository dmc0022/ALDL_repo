...
module home_renderer #(
    ...
    // TEMP icon position (bottom-center)
    localparam TEMP_ICON_X0 = (GAME_W - ICON_W)/2;          // 136
    localparam TEMP_ICON_Y0 = 112;
    localparam TEMP_ICON_X1 = TEMP_ICON_X0 + ICON_W - 1;    // 183
    localparam TEMP_ICON_Y1 = TEMP_ICON_Y0 + ICON_H - 1;    // 159
    ...
    // "TEMP" label under TEMP icon
    localparam TEMP_LABEL_CHARS = 4;
    localparam TEMP_LABEL_W     = TEMP_LABEL_CHARS*CHAR_W +
                                  (TEMP_LABEL_CHARS-1)*CHAR_SPACE;
    localparam TEMP_LABEL_X0    = TEMP_ICON_X0 + (ICON_W - TEMP_LABEL_W)/2;
    localparam TEMP_LABEL_Y0    = TEMP_ICON_Y0 + ICON_H + 8;
    ...
    // "TEMP" text inside TEMP icon
    localparam TEMP_TEXT_CHARS = 4;
    localparam TEMP_TEXT_W     = TEMP_TEXT_CHARS*CHAR_W +
                                 (TEMP_TEXT_CHARS-1)*CHAR_SPACE;
    localparam TEMP_TEXT_X0    = TEMP_ICON_X0 + (ICON_W - TEMP_TEXT_W)/2;
    localparam TEMP_TEXT_Y0    = TEMP_ICON_Y0 + (ICON_H - CHAR_H)/2;
    ...
    localparam [15:0] COL_TEMPBOX   = 16'h07FF;
    ...
    always @* begin
        ...
        // ---------- TEMP ICON BOX ----------
        if ( (game_x >= TEMP_ICON_X0) && (game_x <= TEMP_ICON_X1) &&
             (game_y >= TEMP_ICON_Y0) && (game_y <= TEMP_ICON_Y1) ) begin

            if (game_x == TEMP_ICON_X0 || game_x == TEMP_ICON_X1 ||
                game_y == TEMP_ICON_Y0 || game_y == TEMP_ICON_Y1)
                pixel_color = COL_BORDER;
            else
                pixel_color = COL_TEMPBOX;
        end

        // ---------- "TEMP" text inside TEMP icon ----------
        if ( (game_x >= TEMP_TEXT_X0) &&
             (game_x <  TEMP_TEXT_X0 + TEMP_TEXT_W) &&
             (game_y >= TEMP_TEXT_Y0) &&
             (game_y <  TEMP_TEXT_Y0 + CHAR_H) ) begin

            tx = game_x - TEMP_TEXT_X0;
            ty = game_y - TEMP_TEXT_Y0;
        ...
        // ---------- "TEMP" label ----------
        if ( (game_x >= TEMP_LABEL_X0) &&
             (game_x <  TEMP_LABEL_X0 + TEMP_LABEL_W) &&
             (game_y >= TEMP_LABEL_Y0) &&
             (game_y <  TEMP_LABEL_Y0 + CHAR_H) ) begin

            tx = game_x - TEMP_LABEL_X0;
            ty = game_y - TEMP_LABEL_Y0;
